  ...
  if (isCLIProxyInstalled()) {
    ...
    if (isFaulty) {
      return {
        ...
        details: binaryPath,
        fix: `Upgrade: scc cliproxy install ${CLIPROXY_MAX_STABLE_VERSION.replace(/-\d+$/, '')}`,
      };
    }
    ...
    if (isUnstable) {
      return {
        ...
        details: binaryPath,
        fix: `Stable: scc cliproxy install ${CLIPROXY_MAX_STABLE_VERSION.replace(/-\d+$/, '')}`,
      };
    }
  ...
  for (const status of authStatuses) {
    ...
    if (status.authenticated) {
      ...
      checks.push({
        ...
        fix: `Run: scc ${status.provider} --auth`,
      });
    }
  }
