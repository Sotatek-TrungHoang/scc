 ...
 * v17: Persist routing.strategy from SCC unified config
 */
...
function getConfigVersionFromContent(content: string): number | null {
  const versionMatch = content.match(/SCC v(\d+)/);
  if (!versionMatch) {
    return null;
  }
...
function generateUnifiedConfigContent(
  ...
  const config = `# CLIProxyAPI config generated by SCC v${CLIPROXY_CONFIG_VERSION}
...
#       enabled: true
#       request_log: true
# Then run 'scc doctor --fix' to regenerate this config.
# Use 'scc cleanup' to remove old logs.
...
# Enable usage statistics for SCC dashboard analytics
usage-statistics-enabled: true

# Remote management API for SCC dashboard integration
remote-management:
  allow-remote: true
...
# API keys for SCC and user-added external requests
# NOTE: User-added keys are preserved across SCC updates (fix for issue #200)
api-keys:
${apiKeysYaml}
