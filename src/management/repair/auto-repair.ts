...
export async function runAutoRepair(): Promise<void> {
  ...
  if (fixed > 0) {
    ...
    console.log(`  ${color('scc doctor', 'command')}  - for full diagnostics`);
  } else {
    console.log(ok('No issues found that needed fixing'));
    console.log('');
    console.log(info('If you still have issues:'));
    console.log(`  1. Run ${color('scc doctor', 'command')} for diagnostics`);
    console.log(
      `  2. Try ${color('scc <provider> --auth --verbose', 'command')} for detailed logs`
    );
