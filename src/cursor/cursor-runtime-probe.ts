...
export async function probeCursorRuntime(config: CursorConfig): Promise<CursorProbeResult> {
  ...
  if (!authStatus.authenticated || !authStatus.credentials) {
    return {
      ...
      status: 401,
      duration_ms: Date.now() - startedAt,
      message: 'Cursor credentials not found. Run `scc legacy cursor auth` first.',
  ...
  if (authStatus.expired) {
    return {
      ...
      status: 401,
      duration_ms: Date.now() - startedAt,
      message: 'Cursor credentials expired. Run `scc legacy cursor auth` again.',
      error_type: 'authentication_error',
    };
  }
  ...
  if (!daemonRunning) {
    return {
      ...
      duration_ms: Date.now() - startedAt,
      message:
        'Cursor daemon is not running. Start it with `scc legacy cursor start` or enable auto_start.',
