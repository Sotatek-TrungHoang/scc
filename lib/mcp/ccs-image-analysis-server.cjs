...
function formatErrorDetail(filePath, error) {
  ...
  if (message.startsWith('AUTH_ERROR:')) {
    return `ImageAnalysis failed because SCC vision auth for this provider is unavailable (${message.split(':')[1]}).`;
  }
  ...
  if (message.startsWith('API_ERROR:')) {
    return `ImageAnalysis failed at the SCC provider route while analyzing ${path.basename(filePath)}.`;
  }
  ...
  if (
    ...
    return `ImageAnalysis could not reach the configured SCC provider route for ${path.basename(filePath)}.`;
  }
  ...
  if (!shouldExposeTools()) {
    writeResponse(id, {
      content: [
        {
          type: 'text',
          text: 'SCC ImageAnalysis is unavailable for this profile or no provider-backed vision route is ready.',
        },
      ],
      isError: true,
    });
