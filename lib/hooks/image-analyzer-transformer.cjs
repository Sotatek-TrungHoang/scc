#!/usr/bin/env node
/**
 * SCC Image Analyzer Hook - Read Tool Interceptor
 *
...
function formatErrorOutput(filePath, errorCode, message, troubleshooting) {
  ...
  lines.push('');
  lines.push('For help: scc config image-analysis --help');
...
function outputAuthError(filePath, statusCode) {
  ...
  const output = formatErrorOutput(
    ...
    [
      `Re-authenticate: scc ${provider} --auth`,
      `Check accounts: scc ${provider} --accounts`,
...
function outputTimeoutError(filePath, timeoutSec) {
  ...
  const output = formatErrorOutput(
    ...
    [
      ...
      `Increase timeout: scc config image-analysis --timeout ${timeoutSec * 2}`,
...
function outputRateLimitError(filePath, retryAfterSec) {
  ...
  const output = formatErrorOutput(
    ...
    [
      ...
      'Consider switching accounts: scc ' + provider + ' --accounts',
      'Check quota: scc cliproxy doctor',
    ]
  );
  console.log(JSON.stringify(output));
  process.exit(2);
...
function outputApiError(filePath, statusCode, responseBody) {
  ...
  const output = formatErrorOutput(
    ...
    [
      'Check CLIProxy logs: scc cleanup --show-logs',
      'Verify provider is authenticated: scc doctor',
...
function outputUnknownError(filePath, error) {
  const output = formatErrorOutput(
    ...
    [
      ...
      'Verify authentication: scc doctor',
...
function outputCliProxyUnavailableFallback(filePath) {
  ...
  const output = {
    ...
    systemMessage: `[Image Blocked] ${fileName} - CLIProxy unavailable. Start: scc config`,
    hookSpecificOutput: {
      hookEventName: 'PreToolUse',
      permissionDecision: 'deny',
...
async function processHook() {
  ...
  try {
    ...
    if (process.env.CCS_DEBUG) {
      console.error('[SCC Hook] Error:', err.message);
    }
