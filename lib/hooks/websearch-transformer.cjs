#!/usr/bin/env node
/**
 * SCC WebSearch Hook - deterministic search backends with legacy CLI fallback
 *
 * Primary providers:
 *   - Exa Search API
 *   - Tavily Search API
...
function debug(message) {
  if (process.env.CCS_DEBUG) {
    console.error(`[SCC Hook] ${message}`);
  }
}

function getCcsDirPath() {
  // SCC_* env vars take precedence over CCS_* for fork independence
  if ((process.env.SCC_DIR || '').trim()) {
    return path.resolve(process.env.SCC_DIR.trim());
  }
  if ((process.env.CCS_DIR || '').trim()) {
    return path.resolve(process.env.CCS_DIR.trim());
  }

  if ((process.env.SCC_HOME || '').trim()) {
    return path.join(path.resolve(process.env.SCC_HOME.trim()), '.scc');
  }
  ...
  if (home) {
    return path.join(home, '.scc');
  }

  return path.join(process.cwd(), '.scc');
}
...
function buildSuccessHookOutput(query, providerName, content) {
  return {
    hookSpecificOutput: {
      ...
      permissionDecisionReason: `SCC already retrieved WebSearch results locally via ${providerName}. Use the provided context instead of calling native WebSearch for "${query}".`,
...
function buildFailureHookOutput(query, errors) {
  ...
  return {
    hookSpecificOutput: {
      hookEventName: 'PreToolUse',
      permissionDecision: 'deny',
      permissionDecisionReason: `SCC could not complete local WebSearch for "${query}". Native WebSearch is unavailable for this profile.`,
      additionalContext: `SCC local WebSearch failed for "${query}". Attempted providers: ${detail}`,
    },
  };
}
